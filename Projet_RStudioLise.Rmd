...
```{r}
moy<- colMeans(Xnaquanti, na.rm = TRUE)
X_moy=Xnaquanti
ind.na=which(is.na(X_moy),arr.ind=TRUE)
X_moy[ind.na]=moy[ind.na[,2]]
...
df2.locf=na.locf(Xnaquanti,na.rm=FALSE)
df2.locf=na.locf(df2.locf,na.rm=FALSE,fromLast=TRUE) # dans l'autre sens

```

```{r}
library(naniar)
gg_miss_var(df2.locf)
```
