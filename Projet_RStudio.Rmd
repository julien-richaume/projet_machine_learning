...
```{r}
ggplot(data.frame(x=df3$Energy_consumption_log, y=df3$Access.to.electricity....of.population.),aes(x,y))+
geom_point(col='blue')+ggtitle("Relation entre accés à l'electricité et la consommation d'énergie")
ggplot(data.frame(x=df3$Energy_consumption_log, y=df3$Access.to.clean.fuels.for.cooking),aes(x,y))+
geom_point(col='blue')+ggtitle("Relation entre accés au carburant propre pour la cuisine et la consommation d'énergie")
```
...
```{r}
ggplot(data.frame(x=df3$Value_co2_log, y=df3$Primary.energy.consumption.per.capita..kWh.person.),aes(x,y))+
geom_point(col='blue')+ggtitle("Relation entre émission CO2 et conso d'énergies primaires ")
```

```{r}
ggplot(data.frame(x=df3$Value_co2_log, y=df3$Renewable.energy.share.in.the.total.final.energy.consumption....),aes(x,y))+
