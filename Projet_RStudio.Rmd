...
# 2. Modèle de régression linéaire Gaussien sans sélection de variables

Algo pour visualiser le graphe des résidus
```{r}
gplot.res <- function(x, y, titre = "titre"){
    ggplot(data.frame(x=x, y=y),aes(x,y))+
    geom_point(col = "blue")+xlim(0, 20)+ylim(-10, 10)+
...
## Modèle de régression linéaire Gaussien avec sélection de variables
...
```
```{r}
gplot.res <- function(x, y, titre = "titre", xmin, xmax, ymin, ymax){
    ggplot(data.frame(x=x, y=y),aes(x,y))+
