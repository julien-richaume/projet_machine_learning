...
# Normalisation avec preProcess
preproc <- preProcess(df2[, c(3:19)], method = c("center", "scale"))
df2_stand <- predict(preproc, df2[, c(3:19)])
...
## ACP
library(FactoMineR)
ACP_df2 <- PCA(df2_stand, scale.unit = TRUE,
           graph = FALSE, ncp = 7)
g1<-fviz_eig(ACP_df2, addlabels = TRUE, ylim = c(0, 40))
grid.arrange(g1,ncol=1)
     
```

```{r}
# Visualisation des individus sur les deux premières composantes principales
g2 <- fviz_pca_ind(ACP_df2,
                   axes = c(2, 3),
                   col.ind = "cos2", # Color by the quality of representation
                   gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                   repel = TRUE, # Évite le chevauchement des étiquettes
                   addEllipses = TRUE, # Ajoute des ellipses de confiance autour des groupes
                   legend.title = "Cos2")
...
fviz_pca(ACP_df2, axes=c(1,2), geom = c("point"), col.ind="grey")
