...
Erreur de classification pour la régression
```{r}
...
legend("right", legend=c("Logit", "TreeReg"),
       col=c("blue","orange","green"), lty=c(1,2,1), text.font=1,    cex=0.8)

```

Modèle d'agrégation de modèle (Random Forest, Boosting)
# Forets Aléatoires 
...
```{r}
rf.reg=randomForest(Value_co2_log~., data=datappr, ntree=1000,do.trace=50,importance=TRUE, mtry=3)
```

```{r}
#2ème arbre qui prend en compte les données test
rf.reg2 = randomForest(x=datappr, y=datappr$Value_co2_log, xtest=datappr, ytest=datappr$Value_co2_log, ntree=1000,do.trace=50,importance=TRUE, mtry=3)
```

On cherche à minimiser le critère Out-Of-Bag pour trouver le nombre d'arbre adéquat.
Dans le cas 2, les MSE sont bien plus petites.
...
```{r}
reg.rfr.cv=rfcv(datappr, datappr$Value_co2_log, nfold=10)
print(reg.rfr.cv)
```

```{r}
fit.rfr=rf.reg$predicted
fit.rfr.cv=reg.rfr.cv$predicted$'13'
fit.rfr2=rf.reg2$predicted
res.rfr=fit.rfr-datappr$Value_co2_log
res.rfr.cv=fit.rfr.cv-datappr$Value_co2_log
res.rfr2=fit.rfr2-datappr$Value_co2_log
gplot.res(fit.rfr,res.rfr,titre="")
##avec le modèle optimal par validation croisée
gplot.res(fit.rfr.cv,res.rfr.cv,titre="")
gplot.res(fit.rfr2,res.rfr2,titre="")
```
...
pred.rfr2 = rf.reg2$test$predicted
sum((pred.rfr2-datastr$Value_co2_log)^2)/nrow(datastr)
...
```{r}
boost.reg = gbm(Value_co2_log ~ ., data = datappr, distribution = "gaussian", n.trees = 1000, 
...
err=sum((pred.test-datastr)^2)/nrow(datestr)
test=c(test,err)}
...
```{r}
pred.nnetr=predict(nnet.reg,newdata=datastr)
...
```{r}
Xnaquanti=df[,-c(1,2)] #avec valeurs NA
Xquanti=df2[,-c(1,2)] #sans valeur NA
ind.na=which(is.na(Xnaquanti),arr.ind=TRUE)
ind.na.CO2 = which(is.na(Xnaquanti$Value_co2_emissions_kt_by_country),arr.ind=TRUE)
```

Visualisation de la proportion de valeur manquante dans notre dataframe df 
```{r}
library(naniar)
...
Calcul de l'erreur associée

```{r}
err.moy=(Xquanti-X_moy)[ind.na.C02,2]
##problème car dim differentes 
sqrt(mean(err.moy^2))
boxplot(data.frame(err.moy))
```
...
Calcul de l'erreur  
```{r}
err.med=(Xquanti-X.med)[ind.na.CO2,2]
sqrt(mean(err.med^2))
boxplot(data.frame(err.moy, err.med))
```

Complétion par la valeur de la derniere valeur connue
...
Il n'y a donc plus de valeur NA

Calcul de l'erreur 
```{r}
err.locf=(Xquanti-X.locf)[ind.na.C02,2]
##problème car dim differentes 
sqrt(mean(err.locf^2))
boxplot(data.frame(err.moy, err.locf, err.med))
```

Complétion par les k plus proches voisins
...
```{r}
err.kNN=(Xquanti-X.kNN)[ind.na.CO2,2]
sqrt(mean(err.kNN^2))
boxplot(data.frame(err.moy, err.locf, err.med, err.kNN))

```

Complétion par MissForest
...
Erreur associée
```{r}
err.mf=(Xquanti-X.mf)[ind.na.CO2,2]
sqrt(mean(err.mf^2))
boxplot(data.frame(err.moy, err.locf, err.med, err.kNN, err.mf))
```
...
Erreur associée
```{r}
err.amel=(Xquanti-X.amel)[ind.na.CO2,2]
sqrt(mean(err.amel^2))
boxplot(data.frame(err.moy, err.locf, err.med, err.kNN, err.mf, err.amel))
```
